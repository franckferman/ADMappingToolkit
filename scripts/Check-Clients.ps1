<#
.SYNOPSIS
    Enumerates Windows client AD computer accounts and probes network reachability.

.DESCRIPTION
    Check-Clients.ps1 is a wrapper around Get-ClientInventory from ADMappingToolkit.psm1.
    It pre-filters the AD query to Windows client operating systems (Windows 10, 11, 7,
    XP, Vista, 8, 8.1) before scanning, focusing results on end-user workstations.

    Useful for compliance audits (identifying end-of-support clients across the fleet),
    RDP exposure mapping on workstations, inactive machine cleanup, and workstation
...
.PARAMETER Targets
    Override AD query with explicit targets. Accepts: hostnames, wildcards (IT-00*),
...
.PARAMETER CheckWinRM
    Test WinRM availability via Test-WSMan on each reachable workstation.
...
.PARAMETER CheckUnconstrained
    Retrieve TrustedForDelegation from AD. Flag workstations with unconstrained
    Kerberos delegation — unusual and potentially indicative of misconfiguration.

.PARAMETER CheckEOS
    Flag workstations running end-of-support operating systems (Windows XP, Vista, 7,
    8, 8.1) that no longer receive Microsoft security patches. These systems represent
    a permanent and growing vulnerability surface with no remediation path short of
    OS upgrade.
...
.EXAMPLE
    ...
    List all workstations with end-of-support OS and their OU path. No network probes.

.EXAMPLE
    .\scripts\Check-Clients.ps1 -Report OpenRDP -OnlyReachable
    Identify workstations with TCP 3389 (RDP) exposed and reachable.

.EXAMPLE
    .\scripts\Check-Clients.ps1 -MinDaysInactive 60 -ADOnly -AutoCsv
    List workstations inactive for 60+ days and export to CSV.

.EXAMPLE
    .\scripts\Check-Clients.ps1 -OnlyReachable -Turbo -Stats
    Ping all workstations in parallel and print reachability statistics.
...
if ($AutoCsv) {
  ...
  $autoName = "Cartographie-Clients_$timestamp.csv"
  if ($OutCsv) {
...
if ($Stats) {
  ...
  if ($null -ne $up) {
    ...
    "{0} total clients found" -f $total | Write-Host
  }
}
